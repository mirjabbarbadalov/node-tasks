...
const getAllNews = (req, res) => {
  ...
  try {
    res.status(200).json(dataForSend);
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
};

// Controller for getting selected news by ID
...
// Controller for updating news by ID
const updateNews = (req, res) => {
  const idToUpdate = +req.params.id;
  const modifiedNews = req.body;

  try {
    const indexToUpdate = newsData.findIndex((news) => news.id === idToUpdate);

    if (indexToUpdate === -1) {
      res.status(404).json({ error: "News not found" });
    } else {
      newsData[indexToUpdate] = { ...newsData[indexToUpdate], ...modifiedNews };
      res.status(200).json(newsData[indexToUpdate]);
    }
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
...
const deleteNews = (req, res) => {
  ...
  const indexToDelete = newsData.findIndex((news) => news.id === idToDelete);
  ...
  try {
    if (indexToDelete === -1) {
      ...
      res.status(200).json({ message: `News with ID ${idToDelete} deleted` });
    }
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
...
export { getAllNews, getNewsById, addNews, updateNews, deleteNews };
